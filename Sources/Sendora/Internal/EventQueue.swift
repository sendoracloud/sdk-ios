...
final class EventQueue {
    ...
    init(storage: SendoraStorage, flushAt: Int = 20, maxSize: Int = 1000) {
        ...
        if !persisted.isEmpty {
            ...
            SendoraLogger.shared.debug("Loaded \(persisted.count) persisted events")
        }
    }
    ...
    func persistToDisk() {
        queue.async { [weak self] in
            ...
            SendoraLogger.shared.debug("Persisted \(self.events.count) events to disk")
        }
    }
    ...
    private func performFlush() {
        ...
        SendoraLogger.shared.debug("Flushing \(batch.count) events")
        flushHandler?(batch)
    }
