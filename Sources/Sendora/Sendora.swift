...
/// Sendora iOS SDK — deep linking, attribution, event tracking.
///
/// ```swift
/// Sendora.configure(apiKey: "pk_live_...", projectId: "<uuid>")
/// Sendora.consent.grant()
///
/// // When you have an HMAC-signed identity token from your backend:
/// Sendora.identify(userId: "user_123",
///                  traits: ["email": "a@b.co"],
///                  options: SendoraIdentifyOptions(identityToken: "..."))
///
/// Sendora.trackEvent("purchase", properties: ["amount": 29.99])
/// ```
///
/// Security notes
...
public final class Sendora {
    ...
    private static var config: SendoraConfig?
    ...
    public static let consent = SendoraConsent(initial: false)
    ...
    public static func configure(apiKey: String, projectId: String, options: SendoraConfig? = nil) {
        let cfg = options ?? SendoraConfig(apiKey: apiKey, projectId: projectId)
        let finalConfig = SendoraConfig(
        ...
        do {
            try SendoraValidator.validateApiKey(finalConfig.apiKey)
            try SendoraValidator.validateApiUrl(finalConfig.apiBaseUrl)
        } catch {
            SendoraLogger.shared.error("\(error)")
            return
        }

        serialQueue.sync {
            self.config = finalConfig
            SendoraLogger.shared.isEnabled = finalConfig.debug
        ...
        SendoraLogger.shared.debug("Configured — project: \(projectId)")
    ...
    public static func handleDeepLink(url: URL) -> SendoraLinkData? {
        guard isConfigured, let config = config else {
            SendoraLogger.shared.error("SDK not configured. Call configure() first.")
            return nil
        }
        ...
        SendoraLogger.shared.debug("Deep link handled")
    ...
    public static func checkDeferredDeepLink(completion: @escaping (SendoraLinkData?) -> Void) {
    ...
    public static func trackEvent(_ name: String, properties: [String: Any]? = nil) {
        ...
        do {
            try SendoraValidator.validateEventName(name)
            try SendoraValidator.validateProperties(properties)
        } catch {
            SendoraLogger.shared.error("\(error)")
            return
        }
    ...
    public static func identify(userId: String, traits: [String: Any]? = nil, options: SendoraIdentifyOptions? = nil) {
        guard isConfigured else { return }
        guard !userId.isEmpty, userId.count <= 256 else {
            SendoraLogger.shared.error("userId must be 1-256 chars")
            return
        }
        serialQueue.sync {
    ...
    public static func reset() {
        ...
        SendoraLogger.shared.debug("Identity reset")
    }
