...
public enum SendoraError: Error, CustomStringConvertible {
    ...
    public var description: String {
        switch self {
        case .invalidApiKey(let m): return "[Sendora] invalid apiKey: \(m)"
        case .invalidApiUrl(let m): return "[Sendora] invalid apiUrl: \(m)"
        case .invalidEventName(let m): return "[Sendora] invalid event name: \(m)"
        case .payloadTooLarge(let n): return "[Sendora] payload too large: \(n) bytes"
        case .payloadTooDeep(let n): return "[Sendora] payload nested too deep: \(n) levels"
        case .forbiddenKey(let k): return "[Sendora] forbidden property key: \(k)"
        }
    }
}

enum SendoraValidator {
